...
# 🔹 Use the scraping logger for AI extraction steps
...
class SearchAgent:
    ...
    def extract_company_profile(
        self, company_name: str, enhanced_prompt: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            with self._model_context() as model:
                ...
                logger.debug(f"Raw AI response snippet: {raw_text[:200]}...")
                ...
                validation_logger.info(f"Extraction validated for {company_name}")
            ...
            get_error_logger("validation_errors").error(
                f"JSON parse error for {company_name}: {e}", exc_info=True
            )
            ...
            get_error_logger("extraction_errors").error(
                f"AI extraction failed for {company_name}: {e}", exc_info=True
            )
            raise ExtractionError(
...
"""
    ...
    def _parse_ai_response(self, raw_text: str) -> Dict[str, Any]:
        ...
        logger.warning("No valid JSON found in AI response; using empty profile")
    ...
    def _validate_extraction_result(self, data: Dict[str, Any], company_name: str) -> Dict[str, Any]:
        ...
        if not isinstance(data, dict):
            logger.warning(f"Invalid data type from AI: {type(data)}; returning empty profile")
            return validated
        ...
        non_empty = sum(
            bool(v) if not isinstance(v, list) else len(v)
            for v in validated.values()
        )
        total = len(validated)
        quality_score = non_empty / total
        logger.info(
            f"Extraction quality for {company_name}: "
            f"quality_score={quality_score:.2f}, "
            f"extracted_fields={non_empty}, total_fields={total}"
        )
        return validated

    def _validate_founders_list(self, founders: List[Any]) -> List[Dict[str, Any]]:
        """Validate and clean a list of founder dicts/strings."""
        result = []
        for f in founders:
            if isinstance(f, dict):
                name = str(f.get("full_name", "")).strip()
                if not name:
                    continue
                result.append({
                    "full_name": name,
                    "designation": str(f.get("designation", "")).strip() or None,
                    "role_type": str(f.get("role_type", "Founder")).strip()
                })
